...
# ─── Tabs ─────────────────────────────────────────────────────────────────────
tab1, tab2 = st.tabs(["Calendar & Live Weather", "Manual Entry Prediction"])
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2 — PATIENT DEMAND DASHBOARD
# ══════════════════════════════════════════════════════════════════════════════
with tab2:

    st.markdown(
        "Enter today's conditions to get an immediate patient demand estimate "
        "and risk level for the clinic."
    )
    st.markdown("---")

    # ── Inputs ────────────────────────────────────────────────────────────────
    st.markdown('<div class="section-title">Input Patient Demand Factors</div>', unsafe_allow_html=True)

    col_a, col_b = st.columns(2)

    with col_a:
        st.markdown('<div class="section-title">Exam Period</div>', unsafe_allow_html=True)
        exam_label = st.selectbox(
            "Exam Period",
            options=["No Exam Period", "Exam Period Active"],
            label_visibility="collapsed",
        )
        exam_val = 1 if exam_label == "Exam Period Active" else 0

        st.markdown('<div class="section-title" style="margin-top:1.2rem;">Rainfall Level</div>', unsafe_allow_html=True)
    ...
    with col_b:
        st.markdown('<div class="section-title">Temperature Level</div>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    predict_btn = st.button("Predict Patient Demand", type="primary")
    ...
    if predict_btn:
        ...
        low        = int(prediction * 0.6)
        high       = int(prediction * 1.4)

        # ── KPI tiles ─────────────────────────────────────────────────────────
        st.markdown('<div class="section-title" style="margin-top:0.5rem;">Prediction Result</div>', unsafe_allow_html=True)
        k1, k2, k3 = st.columns(3)
        k1.metric("Predicted Patients", str(prediction))
        k2.metric("Low Scenario",        str(low))
        k3.metric("High Scenario",        str(high))

        st.markdown("---")

        # ── Risk level ────────────────────────────────────────────────────────
        st.markdown('<div class="section-title">Demand Risk Level</div>', unsafe_allow_html=True)
        if prediction < 30:
            st.success("🟢  Low Demand — Normal staffing is sufficient.")
        elif prediction < 70:
            st.warning("🟡  Moderate Demand — Consider calling in additional staff.")
        else:
            st.error("🔴  High Demand — Full staffing and extra medication stock recommended.")

        st.markdown("---")

        # ── Trend chart ───────────────────────────────────────────────────────
        st.markdown('<div class="section-title">Demand Scenario Visualisation</div>', unsafe_allow_html=True)
        st.line_chart({
            "Low Scenario":      [low,        low],
            "Expected Patients": [prediction, prediction],
        ...
        <div class="result-box" style="margin-top:0.5rem;">
            <div style="font-size:0.8rem;font-weight:600;letter-spacing:0.06em;
                        text-transform:uppercase;color:#6c757d;margin-bottom:0.4rem;">
                Interpretation
            </div>
            <div style="font-size:0.92rem;color:#333;">
                On {period_text} with <strong>{rainfall_label.lower()}</strong> rainfall
                and <strong>{temp_label.lower()}</strong> temperatures, the clinic is projected
                to receive approximately <strong>{prediction} patients</strong>.
                The realistic range is <strong>{low} – {high} patients</strong>
                depending on day-to-day variation.
            </div>
        </div>
        """, unsafe_allow_html=True)
